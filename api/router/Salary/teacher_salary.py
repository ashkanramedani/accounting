...
@router.post("/teacher/summary/{sub_course_id}", dependencies=[Depends(RateLimiter(times=1000, seconds=1))], response_model=List[sch.Report])
...
@router.put("/teacher/summary/{form_id}", dependencies=[Depends(RateLimiter(times=1000, seconds=1))], response_model=sch.Report)
