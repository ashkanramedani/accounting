...
router = APIRouter(prefix='/api/v1/deopdown', tags=['Drop Down'])


@router.get("/user", dependencies=[Depends(RateLimiter(times=1000, seconds=1))], response_model=List[sch.user_dropdown])
async def employee_deopdown(db=Depends(get_db), order: sch.Sort_Order = "desc", SortKey: str = "name", employee: bool = True):
