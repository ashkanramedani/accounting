...
@router.get("/search/{form_id}", dependencies=[Depends(RateLimiter(times=10, seconds=5))], response_model=sch.Question_response)
...
async def search_all_question(db=Depends(get_db), page: int = 1, limit: int = 10):
    status_code, result = dbf.get_all_question(db, page, limit)
    if status_code != 200:
        raise HTTPException(status_code=status_code, detail=result)
    return result
