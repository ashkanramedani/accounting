...
async def search_all_subcourse(db=Depends(get_db), page: sch.NonNegativeInt = 1, limit: sch.PositiveInt = 10, order: sch.Sort_Order = "desc"):
...
async def search_subcourse_by_course_id(course_id: UUID, db=Depends(get_db)):
    status_code, result = dbf.get_sub_courses_for_course(db, course_id)
    if status_code not in sch.SUCCESS_STATUS:
        raise HTTPException(status_code=status_code, detail=result)
    return result
