...
def setUp_admin(db: Session):
    try:
        ...
        if not emp:
            UID = uuid.uuid4()
            admin_user = dbm.User_form(user_pk_id=UID, created_fk_by=UID, name="Admin", last_name="Admin", status=1)  # type: ignore[call-arg]
            db.add(admin_user)
            db.commit()
            db.refresh(admin_user)
            emp = admin_user

            admin_role = db.query(dbm.Role_form).filter_by(name="Administrator").first()
            if not admin_role:
                admin_role = dbm.Role_form(created_fk_by=emp.user_pk_id, name="Administrator", cluster="Administrator", status=1)   # type: ignore[call-arg]
                db.add(admin_role)
                db.commit()
