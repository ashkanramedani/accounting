...
def update_survey(db: Session, Form: sch.update_survey_schema):
    try:
        ...
        for question_one, question_two in questions:
            ...
            if question_one:
                Q_1_OBJ = db.query(dbm.Questions_form).filter_by(question_pk_id=questions, deleted=False).first()
                if not Q_1_OBJ:
                    return 400, "Bad Request: Question Not Found"
                Q_1_OBJ.deleted = True
            if question_two:
                Q_2_OBJ = db.query(dbm.Questions_form).filter_by(question_pk_id=questions, deleted=False).first()
                if not Q_2_OBJ:
                    return 400, "Bad Request: Question Not Found"
