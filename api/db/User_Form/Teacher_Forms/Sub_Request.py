...
def Verify_sub_request(db: Session, Form: sch.Verify_Sub_request_schema, status: sch.ValidStatus):
    try:
        ...
        records = (
            db.query(dbm.Sub_Request_form) \
            .filter(dbm.Sub_Request_form.deleted == False, dbm.Sub_Request_form.status != "deleted") \
            .filter(dbm.Sub_Request_form.status != status, dbm.Sub_Request_form.sub_request_pk_id.in_(Form.sub_request_pk_id)) \
            .all()
        )
